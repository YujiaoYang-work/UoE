...
#
# lib_path = r"/home/yujiao/project/build/libexample.so"
# lib = torch.ops.load_library(lib_path)
...
#            for i in range(self.world_size):      #注意：去除这一句可能带来问题
            ...
            for i in range(self.world_size):
                if (len(idx_list[i]) != 0):
                    ...
                    # print(x.shape, "shr", self.weight[i].shape)
                    # sys.exit(0)
                    # print(x.shape, self.weight[i].shape, bias.shape)
                    # sys.exit(0)
                    # print(x.dtype, bias.dtype, self.weight[i].dtype)
                    ...
                    output_.append([])
                # print(output_[0].shape, "dfjviopfj")
                # sys.exit(0)
...
class ColumnParallelLinearWithMoE(torch.nn.Module):
    def __init__(self, input_size, output_size, world_size, bias=True, gather_output=True,
        ...
        if use_cpu_initialization:
            ...
            for i in range(world_size):
                ...
                weight_i = nn.Parameter(torch.empty(
                    self.output_size_per_partition, self.input_size,
                    device=torch.cuda.current_device(), dtype=params_dtype))  #####, dtype=params_dtype
    ...
    def forward(self, input_, idx_list, keep_shape=True, split_head=False):
        ...
        bias = self.bias[0].unsqueeze(1).expand(self.world_size, bhs*seq_len, self.bias[0].shape[-1]).clone() if not self.skip_bias_add else None
        input_ = input_.view(self.world_size, bhs * seq_len, input_.shape[3])
        # print(bias[0].dtype, input_.dtype, self.weight[0].dtype)

        # output_ = torch.ops.ops.multi_expert_linear2(input_, self.weight, bias, self.world_size)

        # print(output_.shape)
        # print(bias[0].dtype, input_.dtype, self.weight[0].dtype)
        # sys.exit(0)
        for i in range(self.world_size):
            bias[i] = bias[i].addmm(input_[i], self.weight[i].t())


        output_ = bias.view(self.world_size, bhs, seq_len, -1)#.to(torch.float16)
            # print("dsfkvjnoijdf", output_.shape, self.weight[0].shape, input_.shape)
            # sys.exit(0)
...
class ColumnParallelLinearWithMoE(torch.nn.Module):
    ...
    def forward(self, input_, idx_list, keep_shape=True, split_head=False):
        ...
        with profiler.profile(use_cuda=True, record_shapes=False, profile_memory=True) as prof:
            ...
            bias = self.bias[0].unsqueeze(1).clone().expand(self.world_size, bhs*seq_len, self.bias[0].shape[-1]).clone() if not self.skip_bias_add else None
            # out = bias.unsqueeze(1).expand(self.world_size, bhs*seq_len, self.bias[0].shape[-1]).clone()



            input_ = input_.view(self.world_size, bhs * seq_len, input_.shape[3])
            # print(bias[0].dtype, input_.dtype, self.weight[0].dtype)
            # output = torch.zeros_like(bias)

            output_ = torch.ops.ops.linear2(input_, self.weight, bias, self.world_size)
            ...
            # output = []
            # for i in range(self.world_size):
            #     # output.append(bias[i].addmm(input_[i], self.weight[i].t()))
            #     output.append(F.linear(input_[i], self.weight[i], bias[i]))
            #
            #     # bias[i] = F.linear(input_[i], self.weight[i], bias[i])
            #
            #     # x = F.linear(input_[i], self.weight[i], bias[i])
            #     # print(x[:3, :5])
            #     # bias = bias.unsqueeze(1).expand(self.world_size, bhs*seq_len, self.bias[0].shape[-1]).clone()
            #     # x2 = F.linear(input_[i], self.weight[i], bias[i])
            #     # print(x2[:3, :5])
            #     # sys.exit(0)
            #
            #
            ...
            #####################################################################
            output_ = output_.view(self.world_size, bhs, seq_len, -1)#.to(torch.float16)
            # print("dsfkvjnoijdf", output_.shape, self.weight[0].shape, input_.shape)
            # sys.exit(0)
        prof.export_chrome_trace("/home/yujiao/a1/profile.json")
        # print("dfvjufgv")
...
class ColumnParallelLinearWithMoE1(torch.nn.Module):
    def __init__(self, input_size, output_size, world_size, bias=True, gather_output=True,
                 init_method=init.xavier_normal_, stride=1,
                 skip_bias_add=False, use_cpu_initialization=False,
                 params_dtype=torch.float32):
    ...
    def forward(self, input_, idx_list, keep_shape=True, split_head=False):
        ...
        if (keep_shape):  # gather会增加额外的index操作和full/zero_操作，会大幅增加计算量（如图）
            ...
            for i in range(self.world_size):
                ...
                #            for i in range(self.world_size):      #注意：去除这一句可能带来问题
                if (len(idx_list[i]) != 0):
                    output[idx_list[i], :seq_len, i * self.output_size:(i + 1) * self.output_size] = F.linear(
                        input_[idx_list[i]], self.weight[i], bias)
            ...
            for i in range(self.world_size):
                if (len(idx_list[i]) != 0):
                    x = input_[i]
                    x = copy_to_model_region(x)
                    bias = self.bias[i].clone() if not self.skip_bias_add else None
                    ...
                    # bias = self.bias[i].unsqueeze(0).unsqueeze(1).expand(bhs, seq_len, self.bias[0].shape[-1]).clone() if not self.skip_bias_add else None
                    ...
                    # sys.exit(0)
                    # print(x.dtype, bias.dtype, self.weight[i].dtype)
                    x = x.view(bhs * seq_len, x.shape[2])
                    # x = F.linear(x, self.weight[i], bias)
                    x = torch.ops.ops.linear2(x, self.weight[i], bias)
                    # print("fjnvidjuf")
                    x = x.view(bhs, seq_len, -1)
                    # x = bias.add_(torch.matmul(x, self.weight[i].t()))
                    # x = bias.addmm(x, self.weight[i].t())



                    # x = x @ self.weight[i].t() + bias
                    # x =  torch.matmul(x, self.weight[i].t()) + bias
    ...
    def forward(self, input_, idx_list, keep_shape=True, split_head=False):
        ...
        if (keep_shape):  # gather会增加额外的index操作和full/zero_操作，会大幅增加计算量（如图）
            ...
            for i in range(self.world_size):
                ...
                # print(output_[0].shape, "dfjviopfj")
                # sys.exit(0)
            output_ = torch.stack(output_, dim=0)
            # prof.export_chrome_trace("/home/yujiao/a1/profile.json")
            return output_, self.bias
...
###注意：原版本中使用torch.baddbmm，不但有乘积还有乘以一个权重并加上一个偏差的操作，并且效率更高，后面应当写出这一版本
    ...
    if (keep_shape):
        ...
        output_ = []
        for i in range(world_size):
            if (len(input1[i]) != 0):        #为了加快速度，len(idx_list[i])可以直接通过参数传递获得
...
def ColumnParallelMatmulWithMoE(input1, input2, world_size, idx_list, keep_shape=True, combine_head=True):
    ...
    if (keep_shape):
        ...
        for i in range(world_size):                #！！！！！！有一个显著的问题：如果要多线程，就不能够各个线程公用一个变量，也就是说，
            if (len(input1[i]) != 0):            #所有的变量应该设为一个长度为4的变量数组！！！切记！！否则将会由于锁而阻碍进程
