...
class SparseMLP(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # print(self.use_base_layer, self.use_topk, self.use_threshold, self.use_hash_layer)
...
class ParallelMLP(MegatronModule):
    ...
    def __init__(self, args, config, moe=False, enable_expert_tensor_parallelism=False):
        ...
        print(in_hidden_size, ffn_hidden_size, "fkvdoifjvoidjfivjdifjvid")
...
class ExpertMLPs(MegatronModule):
    ...
    def __init__(self, args, config, moe=False, num_experts=1, enable_expert_tensor_parallelism=False):
...
class ParallelAttention(MegatronModule):
    ...
    def __init__(self, args, config, layer_number,
...
class BasicParallelTransformerLayer(MegatronModule):
    ...
    def __init__(self, args, config,
        ...
        self.apply_residual_connection_post_layernorm \
            = False #config.apply_residual_connection_post_layernorm

        self.bf16 = config.bf16
        self.fp32_residual_connection = True #config.fp32_residual_connection

        # Layernorm on the input data.
        self.input_layernorm = LayerNorm(config.hidden_size, config.layernorm_epsilon)
    ...
    def forward(self, hidden_states, attention_mask=None,
        ...
        if self.num_experts == 1:
            ...
            #注意：原MoE代码中的下列语句无法执行，因为args.iteration不存在，也不方便直接定义，因为iertation是动态变化的
            # mlp_output, moe_loss, _, gate_info = self.mlp(layernorm_output, now_training_process=float(args.iteration) / args.train_iters, enc_input_ids=enc_input_ids)

            #为了方便，这里将args.iertation设置为1
            mlp_output, moe_loss, _, gate_info = self.mlp(layernorm_output, now_training_process=1, enc_input_ids=enc_input_ids)
